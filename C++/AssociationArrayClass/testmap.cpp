...
#include "map_template.h"		//defines template map_template<Kye,Value>
#include "book.h"
...
int main(void)
{
	...
	//Book Database
	map_template<string, Book> BookDatabase;

	BookDatabase.Add("Autostopem Przez Galaktyke", Book("Douglas Adams", ScienceFiction, NaPolce, 145));
	BookDatabase.Add("Imie Wiatru", Book("Patrick Rothfuss", Fantasy, Wypozyczona, 750));
	cout << BookDatabase << endl;

	Book *pB = BookDatabase.Find("Imie Wiatru");
	pB->Category = Comedy;
	cout << BookDatabase;
